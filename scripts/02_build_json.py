...
def pct(n,d): return int(round((n/d)*100)) if d else 0
...
def compute_score(row):
    points=0.0; total=5.0
    cg=norm(row.get("compr_genre")).lower()
    if cg in {"bonne","bon","good"}: points+=1
    elif cg in {"moyenne","average"}: points+=0.5
    if is_yes(row.get("diff_sexe_genre")): points+=1
    gb=norm(row.get("genre_biologique")).lower()
    if gb in {"faux","false"}: points+=1
    if is_yes(row.get("politiques_genre_connaissance")): points+=1
    if is_yes(row.get("gtg_connaissance")): points+=1
    return int(round((points/total)*100))

def main():
    csv=Path("data/raw/submissions.csv")
    if not csv.exists(): raise SystemExit("Missing data/raw/submissions.csv")
    meta=load_meta()
    min_labels=meta.get("ministere_labels", {})
    obs_labels=meta.get("obstacle_labels", {})

    df=decode_csv(csv)
    for c in ["ministere","sexe","fonction"]:
        if c not in df.columns: raise SystemExit(f"Missing column: {c}")

    df["_ministere_label"]=df["ministere"].apply(lambda x: min_labels.get(norm(x), norm(x)))
    ...
    def has_pf(g):
        nb=int(pd.to_numeric(g["nb_points_focaux"], errors="coerce").fillna(0).max()) if "nb_points_focaux" in g.columns else 0
    ...
    if "start" in df.columns:
        ...
        if len(s): periode=f"{s.min().date().isoformat()} → {s.max().date().isoformat()}"
    ...
    ministeres=df.groupby("_ministere_label").apply(agg_min).reset_index(drop=True)
    ministeres=ministeres.sort_values(["score","formes_genre"], ascending=[False, False]).to_dict(orient="records")
    ...
    sexe=dist(df["sexe"]) if "sexe" in df.columns else []
    fonction=dist(df["fonction"]) if "fonction" in df.columns else []
    ...
    sb.columns=["label","value"]; score_bins=sb.to_dict(orient="records")
    ...
    if obs_cols:
        for c in obs_cols:
            code=c.replace("obstacles/","").upper()
            v=int(pd.to_numeric(df[c], errors="coerce").fillna(0).sum())
            obstacles.append({"label": obs_labels.get(code, code), "value": v})
        ...
        for v in df["obstacles"].dropna().astype(str):
            for tok in v.split():
                tok=tok.strip().upper()
                if tok: cnt[tok]+=1
        obstacles=[{"label": obs_labels.get(k,k), "value": v} for k,v in cnt.most_common()]
